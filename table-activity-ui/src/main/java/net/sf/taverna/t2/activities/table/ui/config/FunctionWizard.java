...
import net.sf.taverna.t2.activities.table.operator.TableOneVariableOperator;
import net.sf.taverna.t2.activities.table.operator.TableTwoVariableOperator;
import net.sf.taverna.t2.activities.table.ui.serviceprovider.TableServiceDesc;
import net.sf.taverna.t2.activities.table.ui.textfield.ColumnIdTextField;
import net.sf.taverna.t2.activities.table.ui.textfield.DoubleTextField;
...
public class FunctionWizard extends JDialog{
    ...
    private FunctionWizard(String title, boolean anyValue){
        setTitle(title);
        this.setIconImage(TableServiceDesc.stiltsIcon().getImage());
    ...
    private void addOneVariableOperatorRow(){
        if (anyValue){
            ...
            final JComboBox oneSelector = new JComboBox(TableOneVariableOperator.values());
            oneSelector.setRenderer(listCellRenderer);
            byOne.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e)
                {
                    TableOneVariableOperator operator = (TableOneVariableOperator)oneSelector.getSelectedItem();
    ...
    private void addTwoVariableOperatorRow(){
        ...
        addNextRow(byTwo);
        TableTwoVariableOperator[] values;
        if (anyValue){
            values = TableTwoVariableOperator.values();
        } else {
            values = TableTwoVariableOperator.booleanValues();
        }
        ...
        byTwo.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e)
            {
                TableTwoVariableOperator operator = (TableTwoVariableOperator)twoSelector.getSelectedItem();
